            ...
            <tbody data-flux-rows>
                @forelse ($policies as $policy)
                    <flux:table.row :key="$policy->id">
                        <flux:table.cell variant="strong">{{ $policy->name }}</flux:table.cell>
                        <flux:table.cell>
                            <flux:badge :color="$policy->status === 'active' ? 'green' : 'red'">
                                {{ ucfirst($policy->status) }}
                            </flux:badge>
                        </flux:table.cell>
                        <flux:table.cell align="end">{{ is_array($policy->rules) ? count($policy->rules) : 0 }}</flux:table.cell>
                        <flux:table.cell align="end">
                            <div class="flex justify-end gap-2">
                                <flux:button size="sm" variant="ghost" wire:click="startEditing('{{ $policy->id }}')">
                                    Edit
                                </flux:button>
                                <flux:button
                                    size="sm"
