...
<div class="flex flex-col gap-6">
    <div class="flex flex-col gap-2">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>{{ $event->product?->name ?? 'Event' }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>

        <flux:heading size="lg">{{ $event->product?->name ?? 'Event' }}</flux:heading>
        <flux:text>Adjust publish state and capacity for this event.</flux:text>
    </div>

    <flux:card>
        <div class="flex flex-wrap gap-4 text-sm">
            <flux:link href="{{ route('partner.availability.events.overrides', $event) }}" wire:navigate>
                Manage overrides
            </flux:link>
        </div>
    </flux:card>
    ...
    <flux:card class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="space-y-1">
                ...
                <flux:text>{{ $event->ends_at?->format('M d, Y H:i') ?? '—' }}</flux:text>
            </div>
            <div class="space-y-1">
