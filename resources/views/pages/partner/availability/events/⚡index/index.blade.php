        ...
        <form wire:submit="createEvent" class="grid gap-4 lg:grid-cols-6">
            <flux:field class="lg:col-span-2">
                ...
                <flux:select wire:model.live="createProductId">
                    ...
                    @foreach ($this->products as $product)
                        <flux:select.option value="{{ $product->id }}" wire:key="event-product-{{ $product->id }}">
                            {{ $product->name }}
                        </flux:select.option>
                    @endforeach
                </flux:select>
            ...
            <flux:field class="lg:col-span-2">
                ...
                <flux:select wire:model.live="createEventSeriesId">
                    ...
                    @foreach ($this->eventSeries as $series)
                        <flux:select.option value="{{ $series->id }}" wire:key="event-series-{{ $series->id }}">
                            {{ $series->name }}
                        </flux:select.option>
                    @endforeach
                </flux:select>
                    ...
                    <flux:table.row :key="$event->id">
                        <flux:table.cell variant="strong">
                            <div class="flex flex-col">
                                <flux:link href="{{ route('partner.availability.events.show', $event) }}" wire:navigate>
                        ...
                        <flux:table.cell>{{ $event->starts_at?->format('M d, Y') ?? '—' }}</flux:table.cell>
                        <flux:table.cell>
                        ...
                        <flux:table.cell>
                            <flux:badge :color="$this->publishStateColor($event->publish_state)">
                                {{ ucfirst($event->publish_state) }}
                            </flux:badge>
                        </flux:table.cell>
                        <flux:table.cell>
                            <flux:badge :color="$this->trafficLightColor($event->traffic_light)">
                                {{ ucfirst($event->traffic_light ?? '—') }}
                            </flux:badge>
                        </flux:table.cell>
                        <flux:table.cell align="end">{{ $event->capacity_total ?? '—' }}</flux:table.cell>
                        <flux:table.cell align="end">{{ $event->capacity_reserved }}</flux:table.cell>
                        <flux:table.cell align="end">{{ $available ?? '—' }}</flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row>
