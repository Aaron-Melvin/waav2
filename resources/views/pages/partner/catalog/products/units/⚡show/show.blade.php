...
<div class="flex flex-col gap-6">
    <div class="flex flex-col gap-2">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="{{ route('partner.catalog.products.units.index', $product) }}" wire:navigate>
                Units
            </flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $unit->name }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>
            ...
            <tbody data-flux-rows>
                @forelse ($calendars as $calendar)
                    <flux:table.row :key="$calendar->id">
                        <flux:table.cell variant="strong">{{ $calendar->date?->format('M d, Y') }}</flux:table.cell>
                        <flux:table.cell>{{ $calendar->is_available ? 'Available' : 'Blocked' }}</flux:table.cell>
                        <flux:table.cell align="end">{{ $calendar->min_stay_nights ?? '—' }}</flux:table.cell>
                        <flux:table.cell align="end">{{ $calendar->max_stay_nights ?? '—' }}</flux:table.cell>
                        <flux:table.cell>{{ $calendar->reason ?? '—' }}</flux:table.cell>
